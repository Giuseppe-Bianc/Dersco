...
public final class CompileCommand implements Callable<Integer> {
  ...
  @Option(
      names = {"-O", "--optimize"},
      paramLabel = "LEVEL",
      description = "Optimization level: ${COMPLETION-CANDIDATES} (default: ${DEFAULT-VALUE}).",
      defaultValue = "NONE")
  ...
  public Integer call() {
    ...
    final CompilationRequest request =
        new CompilationRequest(
            inputFile, outputFile, optimizationLevel, emitIntermediateCode, diagnostics);
  ...
  /**
   * Validates the existence/readability of the input file at execution time (it is not possible to
   * do so during parsing with a pure {@code ITypeConverter}, because that would require file system
   * access). Using {@link ParameterException} guarantees the same error format and usage help as
