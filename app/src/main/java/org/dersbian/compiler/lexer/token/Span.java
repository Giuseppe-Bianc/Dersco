...
public record Span(SourceLocation start, SourceLocation end) {
  ...
  public Span merge(final Span other) {
    ...
    final SourceLocation mergedEnd = this.end.offset() >= other.end.offset() ? this.end : other.end;
  ...
  public String extractFrom(final CharSequence source) {
    ...
    return source
        .subSequence(Math.toIntExact(start.offset()), Math.toIntExact(end.offset()))
        .toString();
  }
  ...
  public String toString() {
    ...
    if (start.line() == end.line()) {
      result = "%d:%d-%d".formatted(start.line(), start.column(), end.column());
    } else {
      result = "%d:%d-%d:%d".formatted(start.line(), start.column(), end.line(), end.column());
    }
    return result;
  }
}
