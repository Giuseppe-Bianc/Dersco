...
public final class NumericParsers {
  ...
  private static SuffixResult tryTail3Suffix(final String body) {
    ...
    if (body.length() < MIN_TAIL3_LENGTH) {
      ...
      if ("i32".equals(tail3)
          ...
          || "i16".equals(tail3)
          || "u16".equals(tail3)) {
        found = new SuffixResult(body.substring(0, body.length() - MIN_TAIL3_LENGTH), tail3);
      } else {
        found = null;
      }
    }
  ...
  private static SuffixResult tryTail2Suffix(final String body) {
    ...
    if (body.length() < MIN_TAIL2_LENGTH) {
      ...
      if ("i8".equals(tail2) || "u8".equals(tail2)) {
        found = new SuffixResult(body.substring(0, body.length() - MIN_TAIL2_LENGTH), tail2);
      } else {
        found = null;
      }
    }
  ...
  /**
   * Extracts an optional type suffix from the numeric literal, returning the stripped body and the
   * detected suffix (or {@code null} if none was found).
   *
