...
public class Lexer {
    ...
    private void scanNumber() {
        ...
        if (!cursor.isAtEnd() && cursor.peekCodePoint() == '.') {
            // ^^^ Also guard with isAtEnd() before peekCodePoint()
            literal.appendCodePoint(cursor.peekCodePoint());
            cursor.advance();

            // Remove the redundant if-guard; while handles the zero-digit case cleanly
            while (!cursor.isAtEnd() && Character.isDigit(cursor.peekCodePoint())) {
                literal.appendCodePoint(cursor.peekCodePoint());
                cursor.advance();
            }
    ...
    private void scanNumericLiteralFromFraction(final SourceLocation start) {
        ...
        // Consume fractional digits (at least one required, guaranteed by caller)
        while (!cursor.isAtEnd() && Character.isDigit(cursor.peekCodePoint())) {
            literal.appendCodePoint(cursor.peekCodePoint());
            cursor.advance();
        }
        ...
        // Optional suffix: [uUfFdD] or [iIuU](8|16|32)
        scanNumericSuffix(literal);
...
**Context**
Details of the exercise (exer) are provided separately.
