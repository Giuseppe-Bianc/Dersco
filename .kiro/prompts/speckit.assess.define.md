...
- **Require a substantive problem to define.** When both `intake.md` and `research.md` are absent, proceed only if `(the user will provide the argument in this conversation)` carries real idea/problem text beyond the slug and options. If the input is *only* a slug, do **not** manufacture a definition from it: ask the user for the idea (interactive) or stop with a note (automated).
