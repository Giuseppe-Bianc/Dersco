...
- **Require a substantive idea to research.** If `intake.md` is absent, you may proceed only when `(the user will provide the argument in this conversation)` carries real idea text beyond the slug and options. If the input is *only* a slug (e.g. `slug=offline-mode`), do **not** infer an idea from the slug: ask the user for the idea (interactive) or stop with a note that there is nothing to research (automated).
